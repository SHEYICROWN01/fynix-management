    ...
    auditLogs.map((log) => {
      ...
      return (
        ...
        <div className="flex items-center gap-2" >
          <span
                                className={
        `inline-flex h-6 w-6 items-center justify-center rounded-full ${getSeverityStyles(
          log.severity
        )}`
      }
                              >
            ...
            Unusual activities and potential security threats
              ...
              < div className = "divide-y divide-border" >
              {
                securityEvents.map((event) => (
                  ...
                  <div
                      className={`rounded-lg p-2 ${event.status === "resolved"
                    ? "bg-success/10"
                    : "bg-muted"
                  }`}
                >
    ...
    <div className="flex items-start justify-between" >
      ...
      <p className="font-medium" > { event.type } </p>
        < p className = "text-sm text-muted-foreground" >
          ...
          < span
className = {`rounded-full px-2 py-1 text-xs font-medium capitalize ${event.status === "resolved"
