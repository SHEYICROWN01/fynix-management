      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          ...
          {sparkData && sparkData.length > 1 && (
            <Sparkline data={sparkData} color={sparkColor ?? "#6366f1"} w={64} h={24} />
          )}
        </div>
      </div>
    </div>
...
export default function SmsManagement() {
  ...
  const handleFundSubmit = async () => {
    ...
    try {
      ...
      const charge_per_sms = parseFloat(fundChargePerSms) || fundTarget.charge_per_sms;
          ...
          {instTotalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-3.5 border-t border-border bg-muted/20">
              <p className="text-xs text-muted-foreground">
                Showing <span className="font-semibold text-foreground">{(instPage - 1) * PER_PAGE + 1}–{Math.min(instPage * PER_PAGE, instTotal)}</span> of <span className="font-semibold text-foreground">{instTotal}</span>
              </p>
                  ...
                  return (
                    <div key={item.institution_id} className="flex items-center gap-3 group">
                      ...
                      <div className={`h-8 w-8 rounded-xl bg-gradient-to-br ${avatarGradient(item.institution_name)} flex items-center justify-center shrink-0 shadow-sm`}>
                        <span className="text-[10px] font-bold text-white">{initials(item.institution_name)}</span>
                      </div>
                ...
                {bulkResult.results.map((r) => {
                  ...
                  return (
                    <div key={r.institution_id} className="flex items-center justify-between rounded-xl bg-muted/30 px-4 py-2.5 text-sm">
                      <div className="flex items-center gap-2">
                        {inst && <div className={`h-6 w-6 rounded-lg bg-gradient-to-br ${avatarGradient(inst.name)} flex items-center justify-center`}><span className="text-[9px] font-bold text-white">{initials(inst.name)}</span></div>}
                    ...
                    return (
                      <div key={inst.id} className="flex items-center gap-3 rounded-xl border border-border px-3 py-2.5">
                        <div className={`h-8 w-8 rounded-xl bg-gradient-to-br ${avatarGradient(inst.name)} flex items-center justify-center shrink-0`}>
                          <span className="text-[10px] font-bold text-white">{initials(inst.name)}</span>
                        </div>
                        <div className="flex-1 min-w-0">
